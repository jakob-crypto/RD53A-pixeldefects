...
int main(int argc, char *argv[]) { //./plotWithRoot_Occupancy path/to/directory file_ext
	...
	std::string dir_t, filepath_t, file_name_t, chipnum_t;
	...
	std::string ext = argv[3]; //png, pdf, C, root
	...
	while ((dirp = readdir(dp))) { //pointer to structure representing directory entry at current position in directory stream, and positions directory stream at the next entry. Returns a null pointer at the end of the directory stream.
		...
		if ( strstr(file_path, "OccupancyMap.dat") != NULL) { //if filename contains string declared in argument
			...
			while ((dirp_t = readdir(dp_t)))
			{
				file_name_t = dirp_t->d_name;
				filepath_t = dir_t + "/" + dirp_t->d_name;
				const char *file_path_t = filepath_t.c_str(); 
				
				if (stat(filepath_t.c_str(), &filestat_t)) continue; //skip if file is invalid
				if (S_ISDIR(filestat_t.st_mode)) continue; //skip if file is a directory

				
				std::cout<<"checkpoint2"<<std::endl;
				
				if(strstr(file_path_t, "OccupancyMap-2500.dat")!=NULL)
				{
					
				std::cout <<"Checkpoint3"<<std::endl;
			
				std::cout << "Opening file: " << filepath_t.c_str() << std::endl; 
				
				//TEST
				std::string firstline;
				std::fstream infile_t(filepath_t.c_str(), std::ios::in);
				std::getline(infile_t, firstline);
				std::cout<<firstline<<std::endl;
				
				std::string secondline;
				std::getline(infile_t, secondline);
